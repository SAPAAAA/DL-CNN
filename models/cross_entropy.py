...
class LearnableLabelSmoothingCrossEntropy(nn.Module):
    ...
    def __init__(self, smoothing=0.1, n_component=3):
        ...
        self.mean_pos = Parameter(
            torch.tensor(
                [i / n_component for i in range(n_component + 1)], requires_grad=True
            )
        )
    ...
    def gen_dist(self, probs, pos):
        ...
        for i in range(len(probs)):
            ...
            lr_dist += probs[i] * (
                pos_frac * self.sampler[start_pos].sample([self.n_classes]).cuda()
                + (1 - pos_frac)
                * self.sampler[stop_pos].sample([self.n_classes]).cuda()
            )
        return lr_dist
    ...
    def forward(self, x, target):
        ...
        soft_target = (
            soft_target * self.confidence + self.smoothing * lr_dist / self.n_classes
        )
        # import pdb;pdb.set_trace()
